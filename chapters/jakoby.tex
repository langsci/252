...
\emph{Conditional mood}: In contrast to \emph{że}-clauses, \emph{jakoby}-clauses cannot contain a verbal head to which the conditional\slash subjunctive clitic \emph{by} is attached, triggering a counterfactual interpretation of the embedded proposition:          
...
\ea \ea \gll	a zatem [\textsubscript{DP} krzyk] wielki pobudził wszytek dwor, \textbf{jakoby} krol jż umarł \label{adjunct_argument} \\
    ...
    \z\z		
In addition, \emph{jakoby} could also introduce comparative hypothetical adverbial clauses (= `adverbial clause' in \tabref{staropolski_statystka}):
