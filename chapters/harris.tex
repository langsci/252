...
\citet*{Toledo2011} maintain that both accounts have their merits, but neither is entirely able to account for all the facts (cf.\citet[140]{Toledo2011}). They propose an approach which takes comparison classes (CCs) into account, an approach which also finds application in \citet{Burnett2017} concerning comparison-class based context sensitivity.
...
\ea
	An adjective \textit{P} is \textit{potentially vague} iff there is some context \textit{c} such that \textit{P} gives rise to a Soritical argument in \textit{c} (\textcite[50]{Burnett2017}, emphasis in original). % Check citations - done
\z
...
\ea
	AA\textsuperscript{T}: For all X $\subseteq$ D, \[ \llbracket Q_{\textsuperscript{T}} \rrbracket_X^s = \llbracket Q_{\textsuperscript{T}}\rrbracket_{X}\text{.}\]
\z
\ea
	AA\textsuperscript{P}: For all X $\subseteq$ D, \[\llbracket R_{\textsuperscript{P}}\rrbracket_X^t = \llbracket R_{\textsuperscript{P}}\rrbracket_{X}\text{.}\]
\z
...
\ea
	Sarah is \textit{more pregnant} than Sue; Sarah is showing more.
\z
